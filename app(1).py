...
st.markdown(
    f"""
    <style>
      .stApp {{ background-color: #FFFFFF; color: {COLORS['text']}; }}
      [data-testid="stSidebar"] {{ background-color: {COLORS['gray']}; }}
      .main-title {{ color:{COLORS['navy']}; font-size:2.0rem; font-weight:800; margin-bottom:.2rem; }}
      .subtitle {{ color:#5F6B78; margin-bottom:1.2rem; }}
      .audit-box {{background:{COLORS['gray']}; border-left:5px solid {COLORS['blue']}; padding:16px; border-radius:6px;}}
      .risk-box {{padding:18px; border-radius:8px; color:white; font-weight:700; text-align:center;}}
      div[data-testid="stMetric"] {{background:#F8FAFC; border:1px solid #E5E7EB; padding:12px; border-radius:8px;}}
    </style>
    """,
...
# Métricas.
facturado = fact_p["total"].sum()
cobrado = conc_fact["importe_bancario"].sum()
...
daily = ban_p.assign(
    ingreso=np.where(ban_p["direccion"] == "Ingreso", ban_p["monto_abs"], 0.0),
    egreso=np.where(ban_p["direccion"] == "Egreso", ban_p["monto_abs"], 0.0),
).groupby(ban_p["fecha"].dt.date, as_index=False).agg(ingresos=("ingreso", "sum"), egresos=("egreso", "sum"))
...
if not daily.empty:
    ...
    fig_flow.update_layout(barmode="relative", height=430, yaxis_title="Importe ($)", xaxis_title="Fecha", legend_title="Serie")
